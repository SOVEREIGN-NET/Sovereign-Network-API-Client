 ...
 */
export function validateIdentityId(id) {
 ...
 */
export function validateGuardianId(guardianId) {
 ...
 */
export function validateWalletType(walletType) {
 ...
 */
export function validateProofType(proofType) {
...
function calculateEntropy(str) {
    ...
    if (/[a-z]/.test(str))
        estimatedPoolSize += 26;
    if (/[A-Z]/.test(str))
        estimatedPoolSize += 26;
    if (/[0-9]/.test(str))
        estimatedPoolSize += 10;
    if (/[^a-zA-Z0-9]/.test(str))
        estimatedPoolSize += 32;
 ...
 */
export function validatePassphraseStrength(passphrase, minLength = 16, minEntropy = 60) {
    ...
    if (complexity < 3) {
        throw new Error('Passphrase must include at least 3 of: uppercase letters, lowercase letters, numbers, special characters');
    }
    // Check entropy
    const entropy = calculateEntropy(passphrase);
    if (entropy < minEntropy) {
        throw new Error(`Passphrase entropy too low (${entropy.toFixed(1)} bits). Minimum ${minEntropy} bits required. ` +
 ...
 */
export function constructUrl(base, params) {
